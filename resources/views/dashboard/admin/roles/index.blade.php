    ...
    <div class="space-y-4">
        ...
        <x-table>
            <x-slot name="header">
                ...
                <x-table-th>
                    <x-text>{{ __('Role') }}</x-text>
                </x-table-th>
                <x-table-th>
                    <x-text>{{ __('Total Permission') }}</x-text>
                </x-table-th>
                <x-table-th>
                    <x-text>{{ __('Total Users') }}</x-text>
                </x-table-th>
                <x-table-th>
                </x-table-th>
            </x-slot>
            @if (count($roles) > 0)
                @foreach ($roles as $index => $role)
                    <tr>
                        ...
                        <x-table-td>
                            <x-text>{{ $role->permissions->count() }}</x-text>
                        </x-table-td>
                        <x-table-td>
                            <x-text>{{ $role->users_count }}</x-text>
                        </x-table-td>
                        <x-table-td class="w-16">
                            <div class="flex gap-2">
                                <x-link href="{{ route('dashboard.admin.roles.show', $role->id) }}" aria-label="View role">
                                    <x-icon>
                                ...
                                @if (!in_array($role->name, config('permission.role_permission.protected.edit')))
                                    <x-link href="{{ route('dashboard.admin.roles.edit', $role->id) }}" aria-label="Edit role">
                                        <x-icon>
                                ...
                                @if (!in_array($role->name, config('permission.role_permission.protected.delete')))
                                    <x-link href="#" aria-label="Delete role."
